...
export function GoldPrices() {
  ...
  const getDisplayPrice = (karat: '24k' | '22k' | '21k' | '20k' | '18k' | '16k' | '14k' | '10k') => {
    ...
    } else if (selectedCurrency === 'USD' && data?.source_data.gold_price_usd_per_gram) {
        ...
        <Card className="bg-white shadow-xl">
          <CardHeader className="bg-white border-b">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
              <div>
                <CardTitle className="text-3xl font-bold text-gray-900 mb-2">{t('gold.title')}</CardTitle>
                <p className="text-gray-600">{t('gold.subtitle')}</p>
              </div>

              <div className="flex items-center gap-4 mt-4 lg:mt-0">
                <div className="text-center">
                  <p className="text-xs text-gray-500 uppercase">{t('gold.market')}</p>
                  <p className="text-sm font-semibold text-green-600">
                    {loading ? t('common.loading') : t('common.active')}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-500 uppercase">{t('gold.spread')}</p>
                  <p className="text-sm font-semibold text-gray-800">0.5%</p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-500 uppercase">{t('gold.volume')}</p>
                  <p className="text-sm font-semibold text-gray-800">{t('gold.high')}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-600">{t('gold.currencyDisplay')}</p>
                    <p className="text-lg font-semibold text-gray-800">{t(`currencies.names.${selectedCurrency}`)}</p>
                  </div>
                  <CurrencySelector />
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-600">{t('gold.lastUpdate')}</p>
            ...
            {marketData && (
              <div className="mb-6 text-center">
                <div className="inline-flex items-center gap-4 bg-gray-50 rounded-lg p-4">
                  <div>
                    <span className="text-sm text-gray-600">{t('gold.currentOuncePrice')}</span>
                    <div className="text-2xl font-bold text-gray-800">${marketData.current_price?.toFixed(2)}</div>
                  </div>
                ...
                <tbody>
                  {loading ? (
                    ...
                    goldData.map((item, index) => (
                      <tr key={index} className={item.highlight ? 'bg-amber-100' : 'bg-white'}>
